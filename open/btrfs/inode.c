...
static int btrfs_insert_inode_locked(struct inode *inode)
{
 ...
 #define S_ENCRYPTED_test (1 << 14) /* Encrypted file (using fs/crypto/) */
			    ...
			    unsigned int *trans_num_items)
{
	...
	int ret;
	// let check if function go to fscrypt_setup_filename
	printk("%s : args->orphan: %s\n", __func__,args->orphan);
	if (!args->orphan) {
		ret = fscrypt_setup_filename(dir, &args->dentry->d_name, 0,
					     &args->fname);
		printk("args->fname->disk_name: %s\n", args->fname.disk_name.name);
		printk("args->fname->disk_name.len: %d\n", args->fname.disk_name.len);
		unsigned int test = (dir->i_flags & S_ENCRYPTED_test);
		printk("%llu", test);
		printk("fscrypt_setup_filename is called! Return: ret=%d\n", ret);
		if (ret)
			return ret;
	}
	else {
		printk("fscrypt_setup_filename is not called!\n");
	}
	ret = posix_acl_create(dir, &inode->i_mode, &args->default_acl, &args->acl);
	printk("posix_acl_create retrun : %s (parent=%pS)\n", ret, __builtin_return_address(0));
	if (ret) {
		printk("posix_acl_create return 0!\n");
		fscrypt_free_filename(&args->fname);
		return ret;
	}
	printk("posix_acl_create passed! (parent=%pS)\n", __builtin_return_address(0));
	/* 1 to add inode item */
	*trans_num_items = 1;
			       ...
			       struct inode *inode)
{
	...
	printk("%s: trans_num_items=%d\n", __func__, trans_num_items);
	if (err)
		goto out_inode;
