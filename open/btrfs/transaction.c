		  ...
		  bool enforce_qgroups)
{
	...
	// printk("%s current->journal_info=%s\n", __func__, current->journal_info);
	...
	if (num_items && root != fs_info->chunk_root) {
		...
		if (flush == BTRFS_RESERVE_FLUSH_ALL &&
			...
			printk("%s: Go into flush function (parent=%pS)\n", __func__, __builtin_return_address(0));
		...
		if (need_reserve_reloc_root(root)) {
			printk("%s: Go into if need_reserve_reloc_root function (parent=%pS)\n", __func__, __builtin_return_address(0));
		...
		if (delayed_refs_bytes) {
			printk("%s: Go into if delayed_refs_bytes function (parent=%pS)\n", __func__, __builtin_return_address(0));
		...
		if (rsv->space_info->force_alloc)
			do_chunk_alloc = true;
		printk("%s: check value do_chunk_alloc=%s (parent=%pS)\n", __func__, do_chunk_alloc, __builtin_return_address(0));
