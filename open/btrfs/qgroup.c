			      ...
			      enum btrfs_qgroup_rsv_type type, bool enforce)
{
	...
	if (!test_bit(BTRFS_FS_QUOTA_ENABLED, &fs_info->flags) ||
		...
		{
			printk("%s: Go into if function (parent=%pS)\n", __func__, __builtin_return_address(0));
			printk("%s: check IF value %lu (parent=%pS)\n", __func__, &fs_info->flags, __builtin_return_address(0));
			printk("%s: check IF value %d (parent=%pS)\n", __func__, !test_bit(BTRFS_FS_QUOTA_ENABLED, &fs_info->flags), __builtin_return_address(0));
			printk("%s: check IF value %d (parent=%pS)\n", __func__, !is_fstree(root->root_key.objectid),  __builtin_return_address(0));
			printk("%s: check IF value %d (parent=%pS)\n", __func__, num_bytes == 0, __builtin_return_address(0));
			return 0;
		}
				...
				bool noflush)
{
	...
	printk("%s: btrfs_qgroup_reserve_meta() return ret=%d (parent=%pS)\n", __func__, ret, __builtin_return_address(0));
	if ((ret <= 0 && ret != -EDQUOT) || noflush)
		return ret;
	printk("%s: Go into if try_flush_qgroup (parent=%pS)\n", __func__, __builtin_return_address(0));
	ret = try_flush_qgroup(root);
	if (ret < 0)
		return ret;
	printk("%s: Go into btrfs_qgroup_reserve_meta again (parent=%pS)\n", __func__, __builtin_return_address(0));
